...
* 笔记页：**双视图知识库图集**——默认「图集总览」+ 可切换「时间线」；URL 同步（`view=atlas|timeline`、category/chapter/tag/q/sort）+ 视图/筛选切换淡入上移动画（`.view-swap`）。**图集总览**：每个分类一张 explore-card 图幅卡（SHEET 编号、篇数、章节点线目录 `.leader-dots` 最多 4 行、`+N 章节`、最近更新），点击进入「图幅详情」——章节 chips 横滑（空章节不出现）、按章节分组展示 NoteCard；**时间线**：保留原三级筛选（分类 → 章节面板 grid-rows 0fr→1fr、标签、最新/最早排序）与按年分组（年份标题普通滚动，不吸顶）；列表卡片仍为原「一行一个」横排结构（日期 | 标题 | 类别 chip | 标签 chips | READ →，摘要下一行），hover 上浮 + 角标 + 火花；`/` 聚焦搜索、`Esc` 逐层退出（清搜索→清标签→清章节→清分类→返回图集）；视图偏好存 localStorage 且 URL 参数优先；**返回/浏览器后退逐层回退**（分类/章节/标签/视图层级变化才 `pushState`，打字/排序 `replaceState`，`popstate` 按 URL 还原；图集/时间线都有页内「返回上一级」按钮，与 Esc、浏览器后退同一语义）；章节 chips 横滑行 `pt-2` 预留 hover 上浮空间（2026-09-06）。页头统计条：共 N 篇 · C 个分类 · CH 个章节 · 最近更新 D
* 笔记图集邻近光（2026-09-05，card-proximity-glow.tsx）：鼠标进入带 `data-glow-grid` 的容器后按指针到每张 `.explore-card` 边缘距离写 `--glow-intensity/--glow-x/--glow-y/--glow-radius`；`.glow-card` 专属 `overflow:visible`（全局 `.explore-card` 仍 `overflow:hidden`），`::before` 外扩 `inset:-4px` + `padding:8px`，用 color-mix + mask radial-gradient（mask-composite exclude）只点亮靠鼠标那侧的 8px 环形轮廓并向外凸出；单卡全亮距离 RADIUS×0.35（约 112px）、320px 外熄灭、中间线性衰减，因此附近多卡同时渐变发光；仅 `(hover:hover) and (pointer:fine)` + 非 reduced-motion 启用。作用范围仅限笔记页图集**总览的分类图幅卡**（带 `glow-card` 类）；图幅详情章节组、时间线及其他页面均不带 `data-glow-grid`
* 移动端适配（2026-09-05）：NoteCard `<sm` 改三行——日期 → 标题（line-clamp-2 全宽）→ 类别 chip + READ（标签仍 sm+ 才显示），解决手机上标题只露一两个字/不显示；分类/章节 FilterButton 加 `shrink-0 whitespace-nowrap`，解决图集/时间线章节 chips 在窄屏被 flex 压成竖条的问题
...
* **前台 AI 悬浮助手（2026-09-09，`components/ai-chat.tsx` + `lib/ai-config.ts`）**：右下角悬浮球（fixed 56px、可任意位置拖拽，pointer capture + 6px 位移阈值区分点击/拖动，位置存 localStorage `ez-ai-pos`）；点击展开对话框（自动避开悬浮球放进视口，尺寸 min(视口, 384×600)），流式对话（DashScope OpenAI 兼容 `POST {base}/chat/completions`，模型默认 `deepseek-v4-flash-0731`，忽略 `reasoning_content` 只显示正式回答，Enter 发送 / Shift+Enter 换行 / 停止 / 清空，聊天记录存 localStorage `ez-ai-chat`）；配置来自 `NEXT_PUBLIC_AI_BASE_URL / NEXT_PUBLIC_AI_API_KEY / NEXT_PUBLIC_AI_MODEL`（本地 `.env.local`，GitHub Actions 走 secrets/vars，服务器部署由 `deploy-c2.sh` 读取 `site.config.json` 的 `ai` 字段注入）；admin 页不显示。**安全提醒：浏览器直连会把 API Key 打进静态 JS，公开线上等于公开密钥，仅适合个人站点/低风险场景**
...
* **笔记代码查看/下载面板（2026-09-05，`components/note-code.tsx`）**：笔记 frontmatter 新增 `code`（`/code/xxx.py` 或 https 外链）+ 可选 `codeLabel`；**demo 与 code 可并存**（article-shell 不再互斥），标题右侧出现 DEMO 按钮与 CODE 入口。单文件代码（.py/.sh/.js/.ts/.css/.html 等）点击经 createPortal 打开只读代码侧栏——按扩展名语法高亮（CodeMirror 6 + `@codemirror/lang-python` + `@codemirror/legacy-modes`）、复制、新窗口、关闭；复用 demo 面板的 fixed 右半屏布局与 CSS 变量主题。**`.zip` 源码包不打开编辑器，按钮直接变成「下载源码包 ⭳」**（排除 node_modules/dist 后上传，访客本地 `npm install`）。后台笔记编辑器新增「代码 Code（可选）」字段：`⬆ 上传 .py / .sh / .zip / ...` → `POST /api/code-upload`（admin-server 白名单扩展名含 `.zip`，写入 `public/code/`，重名自动加 `-n` 后缀）自动填地址，另有面板标题输入框。**部署已随 demos 一并处理**：deploy-c2.sh 符号链接 + Nginx `/code/` alias 已就位。

现有 8 个示例（`F:\learn_webgis\05-leaflet\examples` 的 01~08 全部接入）：`leaflet-hello-map`(01 第一张地图)、`leaflet-basemap-switch`(02 底图与瓦片源，依赖 leaflet-providers)、`leaflet-marker-icons`(03 标记与图标)、`leaflet-geojson`(05 GeoJSON 图层)、`leaflet-events`(06 事件与交互，替换了原 04 popup)、`leaflet-layer-controls`(07 图层组织与控件)、`leaflet-plugins`(08 常用插件，依赖 markercluster/heat/minimap/fullscreen 四插件)、`leaflet-popup-click`(04 点击查看详情，已重新挂回第 04 节)。每个 demo 目录只拷依赖的 lib（`lib/leaflet/dist/{leaflet.css,leaflet.js,images/}` + 各插件 dist），不拷整库；1~8 每篇 frontmatter 另挂 `code` → `public/code/0X-*-src.zip`（含 lib 的完整源码包，下载解压后直接打开 index.html）。

* **01-env 接入（2026-09-06）：Vite/Next 源码包 + 构建产物 demo 的完整工作流见演进 38**——单 HTML demo 直接复制到 `public/demos/<名>/`；Vite demo 用 `npm run build -- --base=./` 的 dist 复制进去；源码包 zip 放 `public/code/`（不含 node_modules/dist）；Next demo 同理需静态导出（output:'export'）才能展示。

* **本地整页发白（2026-09-07）**：症状是 Chrome 首页无 CSS（背景/主题变量/布局全失效，只剩 DOM），检查 `layout.css` 返回 404。根因不是内容或 hero-scroll，而是 dev server（PID 从 23:48 启动）运行期间又执行了 `next build`，`.next` 里的 dev CSS 资产被生产构建覆盖/清掉。处理：停掉 dev 进程组 → 把损坏 `.next` 改名为 `.next-broken-20260907` → 重启 `npm run dev` → CSS 200 且首页/背景恢复。**铁律：`npm run dev` 运行时不要执行 `npm run build`；要构建先停 dev，或改用另一个独立构建目录。** 首次打开首页顶部只有等高线+SCROLL 提示、大字在滚动后浮现是设计状态，不是白屏。
...
34. **GEE Playground 改为内置本机登录（2026-09-04，用户反馈「GEE 太麻烦：OAuth 客户端/认证/审核，Error 400: origin_mismatch」）**：弃用「必须自建 OAuth 客户端」路线，改两档认证——① **内置本机登录（默认，localhost / 127.0.0.1）**：用 Google 官方 SDK 内置客户端（Client ID `517222506229-vsmmajv00ul0bs7p89v5m89qs8eb9359.apps.googleusercontent.com` 与配套 secret 均为官方公开常量）走 PKCE + `accounts.google.com` 授权，令牌换到 `oauth2.googleapis.com/token`（CORS 可用）；无需创建 OAuth 客户端、配置来源或申请审核，点「登录 Google」即可，刷新自动续期。② **自定义 OAuth（线上域名）**：内置方式因 Google 只接受 localhost 回调而不可用，才需要在 Cloud Console 建 Web 应用客户端并在 ⚙ 填入；工具页 hint 已同步成「本机免 OAuth，线上才自定义」。**验证**：`node --check app.js` 通过；localhost / 127.0.0.1 回调 200，Google 授权页正常弹出；线上域名 `redirect_uri_mismatch` 属预期，走自定义 OAuth 即可。

35. **GEE Playground 整体移除（2026-09-04，用户最终决定不做 OAuth/审核，删掉这一工具）**：删除 `app/tools/page.tsx` 的 GEE 卡片与 import、`components/tools/gee-playground.tsx` 组件、`public/demos/gee-playground/` 线上 Demo（约 2.3MB，源码可从 git 历史找回）、`content/copy/09-工具页.json` 的 `gee` 段与计数行文案、`lib/site.ts` tools 类型 `gee`、后台 ToolsManager 的 GEE 面板与 `scripts/admin-server.mjs` 的 EMPTY `gee`；保留 `date` 字段（首页勘测路线工具点仍在用）。工具页现在是 格式转换 + DEM 下载器 + 更多规划中。笔记里的 geemap/GEE 学术内容和 GEE 土地变化项目不算工具，未动。

36. **笔记页图集化（2026-09-04，用户嫌「几行筛选+点一下换文章」太平凡）**：把笔记页从简单筛选升级为**双视图知识库图集**——默认图集总览（每个分类一张 SHEET 图幅卡，含 .leader-dots 章节点线目录与最近更新，点击进图幅详情按章节分组）+ 保留的时间线视图（原三级筛选/按年分组）；搜索/章节/标签/视图全走 URL 同步，`/` 聚焦搜索、`Esc` 逐层退出；图集/时间线/章节过滤切换挂 `.view-swap` 淡入上移动画；页头统计条改为「共 N 篇 · C 个分类 · CH 个章节 · 最近更新 D」（`content/copy/02-笔记页.json` page.count + browser 新增键）。**保留铁律**：NoteCard 横排一行一个布局仍不动（用户多轮打磨过，只换外层壳/视图不重排卡片）。**实测 ✓**：桌面 1440 + 移动 390 图集/详情/时间线均无横向溢出；WebGIS 图幅进详情 77 篇、点「Web基础」过滤后 39 篇且 URL/分组正确；Esc 返回图集；控制台零报错。**2026-09-05 修正**：时间线年份标题原 `.year-sticky` 毛玻璃吸顶被用户反馈「一条 2026 一直固定着」，已去掉 sticky 改回普通滚动。

37. **笔记图集邻近发光 + 移动端修复（2026-09-05，用户「卡片靠近的那一端轮廓发光，附近几张同时有；发光加粗往外加粗；手机屏太窄标题只显示一两个字；章节从横的变成挤压的竖条」）**：① 新增 `components/card-proximity-glow.tsx` 全局 pointermove 监听（vanilla + rAF，不引 GSAP）：只处理 `[data-glow-grid]` 容器内的 `.explore-card`，按鼠标到卡片矩形边缘的距离算强度（完全发光 `RADIUS×0.35≈112px`、熄灭 320px、中间线性衰减）写 `--glow-x/--glow-y/--glow-radius/--glow-intensity`；CSS：`.explore-card.glow-card` 专属 `overflow:visible`（全局 `.explore-card` 仍 `overflow:hidden`，研究封面/图集头卡/格式转换卡等继续由各自封面/子元素裁角），`::before` 外扩 `inset:-4px` + `padding:8px`，用 `color-mix` + mask radial-gradient（mask-composite exclude）只点亮靠鼠标方向的 8px 厚环形轮廓并向外凸出，`opacity=--glow-intensity`；`(hover:hover) and (pointer:fine)` + `prefers-reduced-motion` 禁用。`data-glow-grid` 按用户三轮要求最终只挂**笔记页图集总览的分类卡片**（加 `glow-card` 类），图幅详情章节组、时间线及其他页面全部去掉。**实测 ✓**：桌面图集鼠标移到卡片后相邻多卡 `--glow-intensity` 为 1 / 0.791 / 0.265 渐变；图幅详情/时间线/首页均无 `data-glow-grid`。② NoteCard 移动端（<sm）改三行：日期 → 标题（line-clamp-2、全宽）→ 类别 chip + READ →；标签仍 sm+ 显示；摘要 line-clamp-1——手机不再只露一两个字。③ 图集/时间线章节 FilterButton 加 `shrink-0 whitespace-nowrap`，窄屏保持内容宽度横滑，不再被 flex 压成竖条。**实测 ✓**：390px 视口标题宽约 308px、章节 chips 横滑且页面无横向溢出。**教训**：a) 移动端文本 chip 被压成竖条的根因是 flex 子项可压缩——固定/按钮类 chip 一律 `shrink-0 whitespace-nowrap`；b) 卡片发光要向外凸出，光走伪元素外扩，需要发光的那张卡片单独放开 `overflow:hidden`，其余卡片保持原裁角，避免全站卡片回归风险；c) 卡片布局移动端改形态时 sm+ 原横排保持不动（AGENTS.md 19 的铁律）。

38. **01-env 环境与工具链章节接入 + Vite/Next Demo 工作流定稿（2026-09-06，用户「怎么把这个项目放到网站里；不能只上传代码；后面 Vite/Next 怎么演示」）**：① **内容入库**——`content/notes/WebGIS 开发/环境与工具链/` 共 5 篇（order 101-105、date 统一 2026-09-06），frontmatter 用 `category/chapter/order/tags` + `demo/demoLabel/demoHeight/code/codeLabel`；`image/*.png` 与 md 一起复制进同一章节目录，文章首次访问时 `lib/content.ts` 自动复制到 `/content-images/`（内容 md5 命名，需随 git 提交）；md 里原指向 `examples/...` 的练习链接改写成前台真实可用的 `/code/...`（下载）或 `/demos/...`（演示），正文路径描述同步改成解压后的本地目录（如 `bash 02-git-practice.sh`、打开 `05-vite-vanilla/`）。**taxonomy `WebGIS 开发` 的 chapters 数组开头加 `"环境与工具链"`，图集里该章就排最前。**② **代码下载工作流**——单文件（01-check-env.js / 02-git-practice.sh / 03-settings.json）直接复制到 `public/code/`；多文件/Vite 项目打 zip 放 `public/code/`（04-devtools-map-src.zip 含 index.html + lib/leaflet 精简版；05-vite-vanilla-src.zip 含 package.json/package-lock.json/index.html/src/**绝不含 node_modules 与 dist**），前台 `code` 以 `.zip` 结尾时按钮直接变「下载源码包 ⭳」（NoteCode 分支），访客本地解压 → `npm install` → `npm run dev` 即可跑。③ **Demo 工作流**——单 HTML 直接复制到 `public/demos/<名>/index.html` + 需要的 lib；**Vite demo**：外部项目执行 `npm run build -- --base=./`（不动 source package.json），把 `dist/` 整目录复制到 `public/demos/<名>/`，frontmatter `demo: "/demos/<名>/"`；构建产物是压缩合并后的独立页面，**浏览器内「编辑代码」不适用（编辑器只适合单 HTML 的可读源码），只做只读演示**：`--base=./` 让 index.html 里是 `./assets/...` 相对路径，Next dev / GH Pages 子路径 / Nginx 根路径都能跑。**Next demo 同理**：必须 `output:'export'` 静态导出生效，API/SSR 项目静态导不出只能外链或源码包（留给用户后面实操）。④ **前后台改动**——`scripts/admin-server.mjs` `CODE_EXT` 加 `.zip`（code-upload 原样写盘），`components/admin/post-editor.tsx` accept 加 `.zip` 并改文案；`components/article-shell.tsx` 去掉 `!demo &&` 互斥，**demo 与 code 可并存**。**实测**：Vite v8.2.2 构建 122ms 成功，6 模块；两个 zip 均验证不含 node_modules/dist；`next build` 全站构建通过 ✓（编译 22.4s、静态页生成正常、无 TS/内容解析错误）；dev 下 5 篇笔记页全部可访问，7 个 /code 下载与两个 demo 资源全部 200，image/*.png 经文章首访复制进 /content-images/ 后可显示；Chrome 实测两个 iframe 均正常渲染——04 高德瓦片+标记+弹窗、05 Vite 页面；正文 /demos 链接要写带 index.html 的完整地址（Next dev 不做目录解析，生产 GH Pages/Nginx 目录解析正常）。

39. **本地白屏排查定案（2026-09-07）**：用户截图全白，但 DOM 完整；Chrome 实测 `/_next/static/css/app/layout.css?v=...` 返回 404，CSS 变量 `--ink/--bg` 为空、canvas 尺寸退到 300×150、`overflow-x-clip` 也未生效。根因是 dev server 运行时执行生产构建，`.next` 的 dev CSS 被覆盖。停 dev、改名 `.next`、重启后 CSS 200 且首页恢复。没有前台代码回退/修改，因此无需内容 commit；线上旧构建白屏与此是两件事，仍需跑 `sudo bash /srv/my-blog/scripts/deploy-c2.sh` 更新服务器。

40. **前台 AI 悬浮球 + 流式对话（2026-09-09）**：右下角悬浮球任意拖拽（pointer capture、6px 阈值防误触、位置/聊天记录 localStorage），点击展开对话框，直接调 DashScope OpenAI 兼容端点流式对话（模型 `deepseek-v4-flash-0731`，key 走 `.env.local` 不进库）。组件：`components/ai-chat.tsx`；配置：`lib/ai-config.ts`；全站挂在 `app/layout.tsx`（admin 页隐藏）。修复两个拖拽细节：拖完松开不能误开面板（用 `suppressClickRef` 拦住 click）；松开时按 pointerup 坐标算最终位置再存，避免 React 状态未刷新导致存档偏一格。**部署**：GitHub Actions 在 build step 读 `NEXT_PUBLIC_AI_*`（key 用 repo secret）；服务器 `deploy-c2.sh` [3/8] 从 `site.config.json` 的 `ai` 字段读入构建环境，不在脚本里硬编码密钥。**安全提醒**：NEXT_PUBLIC 注入即公开（静态 JS 可被扒 key），个人站可接受，敏感场景应改后端代理。
...
* **前台 AI 配置（2026-09-09）**：本地开发读 `.env.local` 的 `NEXT_PUBLIC_AI_*`（已 gitignore）；GitHub Pages 需在 repo 加 secret `NEXT_PUBLIC_AI_API_KEY`（base/model 用 repository variables）；服务器动态分支由 `deploy-c2.sh` 构建时读取 `/srv/my-blog/site.config.json` 的 `ai` 字段（`baseURL/apiKey/model`）注入，改线上 AI 配置直接改服务器该文件再跑部署脚本
